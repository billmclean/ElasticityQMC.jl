import Elasticity_QMC: double_indices, PDEStore, InterpolationStore, 
...
Interpolating λ and μ using $N₁ x $N₂ spatial grid."""
println(msg)
...
pstore = PDEStore(mesh; conforming=conforming_elements, pcg_tol=pcg_tol)
istore = InterpolationStore(idx, α, N₁, N₂)

integrand_init!(pstore, Λ, μ, ∇μ, f)
z = rand(s) .- 1/2
Φ, Φ_error = integrand!(z, Λ, μ, ∇μ, pstore, istore)

integrand_init!(pstore, λ, f)
y = rand(s) .- 1/2
Φ, Φ_error = integrand!(y, λ, pstore, istore)
